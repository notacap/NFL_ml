"""
NFL Team Offensive Stats Web Scraper

Scrapes NFL team offensive statistics from pro-football-reference.com
Extracts comprehensive team offensive data including passing, rushing, and total offense stats.
"""
...
def scrape_team_offensive_stats(year):
    """
    Scrapes NFL team offensive statistics for a given year.
    
    Args:
        year (int): The year to scrape data for
    
    Returns:
        pd.DataFrame: Processed team offensive stats DataFrame
    """
...
def main():
    ...
    start_scraper_session("tm_off")
    
    try:
        print(f"Scraping NFL team offensive stats for year: {YEAR}")
        
        # Create directory structure
        team_stats_dir = create_directories(YEAR, "tm_off", WEEK_NUMBER)
        print(f"Data will be saved to: {team_stats_dir}")
        
        # Scrape the data
        df = scrape_team_offensive_stats(YEAR)
        
        # Save the data
        if df is not None:
            print(f"Successfully scraped team offensive stats for {len(df)} teams")
            save_data_to_csv(df, team_stats_dir, "tm_off.csv")
        else:
            print("Failed to scrape team offensive stats")
