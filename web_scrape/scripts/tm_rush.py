"""
NFL Team Rushing Stats Web Scraper

Scrapes NFL team rushing statistics from pro-football-reference.com
Extracts comprehensive team rushing data.
"""
...
def scrape_team_rushing_stats(year):
    """
    Scrapes NFL team rushing statistics for a given year.
    
    Args:
        year (int): The year to scrape data for
    
    Returns:
        pd.DataFrame: Processed team rushing stats DataFrame
    """
...
def main():
    ...
    start_scraper_session("tm_rush")
    
    try:
        print(f"Scraping NFL team rushing stats for year: {YEAR}")
        
        # Create directory structure
        team_stats_dir = create_directories(YEAR, "tm_rush", WEEK_NUMBER)
        print(f"Data will be saved to: {team_stats_dir}")
        
        # Scrape the data
        df = scrape_team_rushing_stats(YEAR)
        
        # Save the data
        if df is not None:
            print(f"Successfully scraped team rushing stats for {len(df)} teams")
            save_data_to_csv(df, team_stats_dir, "tm_rush.csv")
        else:
            print("Failed to scrape team rushing stats")
