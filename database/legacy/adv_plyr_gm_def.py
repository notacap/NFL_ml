...
def create_adv_plyr_gm_def_table(db: DatabaseConnector) -> bool:
    """Create the adv_plyr_gm_def table if it doesn't exist."""
    create_table_sql = """
    CREATE TABLE IF NOT EXISTS adv_plyr_gm_def (
        adv_plyr_gm_def_id INT PRIMARY KEY AUTO_INCREMENT,
        plyr_id INT,
        week_id INT,
        game_id INT,
        season_id INT,
    ...
    return create_table_if_not_exists(db, 'adv_plyr_gm_def', create_table_sql)
...
def get_csv_files() -> list:
    ...
    for week in range(WEEK_START, WEEK_END + 1):
        ...
        if os.path.exists(clean_dir):
            pattern = os.path.join(clean_dir, "cleaned_*_gm_plyr_adv_def_*.csv")
...
def process_csv_file(db: DatabaseConnector, file_path: str, season_id: int) -> pd.DataFrame:
    ...
    for index, row in df.iterrows():
        try:
            ...
            # Map database columns to CSV columns based on agent analysis
            stat_mapping = {
                'plyr_gm_def_tgt': 'Tgt',
                'plyr_gm_def_cmp': 'Cmp',
                'plyr_gm_def_cmp_pct': 'Cmp%',  # Special handling needed for percentage
            ...
            # NOTE: The 'Int' column from CSV is NOT inserted into the database
            # This is intentionally excluded per user instructions
            
            # Add coverage stats (these can be empty for non-coverage players)
            for db_col, csv_col in stat_mapping.items():
                if csv_col in df.columns:
                    value = row.get(csv_col)
                    
                    # Handle percentage columns specially
                    if db_col in ['plyr_gm_def_cmp_pct']:
                        processed_row[db_col] = convert_percentage_to_float(value)
                    else:
                        # Handle regular numeric values with better empty value detection
            ...
            # Add pass rush stats (these are always present)
            pass_rush_mapping = {
            ...
            for db_col, csv_col in pass_rush_mapping.items():
                if csv_col in df.columns:
                    value = row.get(csv_col)
                    if pd.notna(value) and value != '' and str(value).strip() != '':
                        try:
            ...
            # Add tackling stats (these are always present) 
            tackling_mapping = {
                'plyr_gm_def_mtkl': 'MTkl',
                'plyr_gm_def_mtkl_pct': 'MTkl%'
                # Note: 'Comb' column from CSV is not mapped - no corresponding column in adv_plyr_gm_def table
            }
            
            for db_col, csv_col in tackling_mapping.items():
                if csv_col in df.columns:
                    value = row.get(csv_col)
                    
                    # Handle percentage columns specially  
                    if db_col in ['plyr_gm_def_mtkl_pct']:
                        processed_row[db_col] = convert_percentage_to_float(value)
                    else:
                        if pd.notna(value) and value != '' and str(value).strip() != '':
                            try:
    ...
    if processed_rows:
        ...
        # Note: According to database schema, these should be INT columns that can be NULL
        nullable_int_columns = ['plyr_gm_def_tgt', 'plyr_gm_def_cmp', 'plyr_gm_def_pass_yds', 
                               'plyr_gm_def_pass_td', 'plyr_gm_def_ay', 'plyr_gm_def_yac',
                               'plyr_gm_def_bltz', 'plyr_gm_def_hrry', 'plyr_gm_def_qbkd',
                               'plyr_gm_def_prss', 'plyr_gm_def_mtkl']
        ...
        # Handle columns that should remain as float (including fractional sacks)
...
def main():
    ...
    print(f"Starting Advanced Player Game Defense Data Import for {YEAR}")
    ...
    try:
        ...
        if not create_adv_plyr_gm_def_table(db):
            return
        ...
        for week, file_path in csv_files:
            try:
                ...
                if not processed_df.empty:
                    success = batch_upsert_data(db, 'adv_plyr_gm_def', processed_df)
                    if success:
