#!/usr/bin/env python3
"""
nfl_fastr_wr.py - NFL FastR Wide Receiver Advanced Metrics Upsert Script

Processes fastr_wr.csv and ngs_wr.csv files to populate nfl_fastr_wr table.
Merges data from both sources on player_display_name, season, week, and team.

Source Directory: web_scrape/scraped_data/nflfastR/receiver_data
"""
...
def create_nfl_fastr_wr_table(db: DatabaseConnector) -> bool:
    """Create the nfl_fastr_wr table if it doesn't exist."""
    create_table_sql = """
    CREATE TABLE IF NOT EXISTS nfl_fastr_wr (
    ...
    """
    return create_table_if_not_exists(db, 'nfl_fastr_wr', create_table_sql)
        ...
        for _, row in season_df.iterrows():
            ...
            try:
                ...
                record = {
                    ...
                    'week_id': week_id,
                    # From fastr_wr.csv
                    'plyr_gm_rec_tgt_share': safe_decimal(row.get('target_share')),
                    'plyr_gm_rec_epa': safe_decimal(row.get('receiving_epa')),
                    'plyr_gm_rec_ay_share': safe_decimal(row.get('air_yards_share')),
                    'plyr_gm_rec_wopr': safe_decimal(row.get('wopr')),
                    'plyr_gm_rec_racr': safe_decimal(row.get('racr')),
                    # From ngs_wr.csv
                    'plyr_gm_rec_avg_cushion': safe_decimal(row.get('avg_cushion')),
                    'plyr_gm_rec_avg_separation': safe_decimal(row.get('avg_separation')),
                    'plyr_gm_rec_avg_yac': safe_decimal(row.get('avg_yac')),
                    'plyr_gm_rec_avg_expected_yac': safe_decimal(row.get('avg_expected_yac')),
                    'plyr_gm_rec_avg_yac_above_expectation': safe_decimal(row.get('avg_yac_above_expectation')),
                    'plyr_gm_rec_pct_share_of_intended_ay': safe_decimal(row.get('percent_share_of_intended_air_yards'))
                }
...
def main():
    """Main function to process FastR WR data."""
    print("=" * 60)
    print("NFL FastR Wide Receiver Advanced Metrics Import")
    print("=" * 60)
    ...
    try:
        ...
        if not create_nfl_fastr_wr_table(db):
            return
        ...
        # Insert/update data
        success = batch_upsert_data(db, 'nfl_fastr_wr', summary['dataframe'])
