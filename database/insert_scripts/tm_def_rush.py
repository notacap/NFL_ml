    ...
    for _, row in tm_def_rush_df.iterrows():
        try:
            ...
            def safe_convert_time(value):
                """Convert time values to TIME format, return None if not available"""
                # Since the CSV doesn't contain time data, return None
                return None
            
            # Note: The CSV file contains rushing defense stats, not drive stats
            # The database schema expects drive stats, but we'll set available values to None
            # since the CSV doesn't contain drive statistics
            record = {
                'team_id': team_id,
                'season_id': season_id,
                'week_id': week_id,
                
                # Drive stats - not available in current CSV, setting to None
                'tm_def_dr': None,  # Drive count not available
                'tm_def_opp_avg_play_dr': None,  # Average plays per drive not available
                'tm_def_opp_net_yds_dr': None,  # Net yards per drive not available
                'tm_def_avg_field_pos': None,  # Average field position not available
                'tm_def_avg_dr_time': None,  # Average drive time not available
                'tm_def_avg_pt_dr': None  # Average points per drive not available
            }
